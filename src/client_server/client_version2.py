...
def draw_boxes_and_tracking(frame, boxes, tracking_info=None):
    ...
    for box in boxes:
        ...
        color = (255, 200, 0) if (tracking_info and 
                                  box.get('id') == tracking_info.get('selected_id') and 
                                  YOLOApp.instance.tracker_type.get() == "YOLO Tracking") else (0, 255, 0)
        thickness = 1
    ...
    # Draw tracking line and box for CSRT
    if (tracking_info and 
        YOLOApp.instance.tracker_type.get() == "CSRT Tracking" and 
        YOLOApp.instance.csrt_bbox is not None):
        x1, y1, x2, y2 = YOLOApp.instance.csrt_bbox
        obj_center = ((x1 + x2) // 2, (y1 + y2) // 2)
        cv2.rectangle(frame_copy, (x1, y1), (x2, y2), (255, 200, 0), 2)
        cv2.line(frame_copy, frame_center, obj_center, (255, 102, 0), 1)
        cv2.circle(frame_copy, obj_center, 3, (255, 102, 0), 1)
    
    # Draw tracking line for YOLO tracking
    elif tracking_info and tracking_info.get('selected_id') is not None:
    ...
    while running:
        if not frame_queue.empty() and YOLOApp.instance and YOLOApp.instance.detection_enabled and YOLOApp.instance.yolo_model:
            try:
                ...
                if YOLOApp.instance.tracker_type.get() == "CSRT Tracking" and YOLOApp.instance.csrt_tracker is not None:
                    # Update CSRT tracker
                    success, bbox = YOLOApp.instance.csrt_tracker.update(frame)
                    if success:
                        x, y, w, h = map(int, bbox)
                        x1, y1, x2, y2 = x, y, x + w, y + h
                        YOLOApp.instance.csrt_bbox = (x1, y1, x2, y2)
                        ...
                        rel_x = dx / (width / 2)
                        rel_y = dy / (height / 2)
                        turn_direction = "RIGHT" if rel_x > 0.1 else "LEFT" if rel_x < -0.1 else None
                        
                        tracking_info = {
                            'selected_id': YOLOApp.instance.selected_tracking_id,
                        ...
                        print("CSRT tracker lost the object")
                        YOLOApp.instance.csrt_tracker = None
                        YOLOApp.instance.csrt_bbox = None
                        YOLOApp.instance.selected_tracking_id = None
                        YOLOApp.instance.tracking_info_label.config(text="Tracking lost")
                        YOLOApp.instance.direction_label.config(text="")
                        YOLOApp.instance.position_label.config(text="")
                ...
                # Initialize CSRT tracker if needed
                if (YOLOApp.instance.tracker_type.get() == "CSRT Tracking" and 
                    YOLOApp.instance.csrt_tracker is not None and 
                    YOLOApp.instance.csrt_bbox is not None and
                    not hasattr(YOLOApp.instance.csrt_tracker, '_tracker_initialized')):
                    YOLOApp.instance.csrt_tracker.init(frame, YOLOApp.instance.csrt_bbox)
                    YOLOApp.instance.csrt_tracker._tracker_initialized = True
                    print("CSRT tracker initialized with frame")
...
class YOLOApp(tk.Tk):
    ...
    def __init__(self):
        ...
        self.tracker_type = tk.StringVar(value="YOLO Tracking")
    ...
    def load_yolo_model(self):
        ...
        model_paths = {
            ...
            "Hybriddet 2.0": "v11_visdrone/cpu/best.pt",
        }
    ...
    def setup_control_panel(self):
        ...
        # Create a new frame for the comboboxes to place them side by side
        combobox_frame = ttk.Frame(detection_frame, style='Dark.TFrame')
        combobox_frame.pack(side=tk.TOP, pady=5)

        self.model_selector = ttk.Combobox(combobox_frame, 
                                         textvariable=self.selected_model, 
                                         values=["YOLOv11n", "Hybriddet 2.0"], 
                                         state="readonly",
                                         style='Modern.TCombobox')
        self.model_selector.pack(side=tk.LEFT, padx=5)
        self.model_selector.bind("<<ComboboxSelected>>", self.model_changed)

        self.tracker_selector = ttk.Combobox(combobox_frame, 
                                           textvariable=self.tracker_type, 
                                           values=["YOLO Tracking", "CSRT Tracking"], 
                                           state="readonly",
                                           style='Modern.TCombobox')
        self.tracker_selector.pack(side=tk.LEFT, padx=5)
        self.tracker_selector.bind("<<ComboboxSelected>>", self.tracker_changed)
        ...
        for box in self.last_boxes:
            if 'id' in box and box['id'] is not None:
                ...
                if x1 <= frame_x <= x2 and y1 <= frame_y <= y2:
                    ...
                    if self.tracker_type.get() == "CSRT Tracking":
                        self.csrt_tracker = cv2.TrackerCSRT_create()
                        self.csrt_bbox = (x1, y1, x2 - x1, y2 - y1)
                        print(f"Initialized CSRT tracker for object ID {self.selected_tracking_id}")
                    else:
                        print(f"Selected object ID {self.selected_tracking_id} for YOLO tracking")
                    break
        
        if self.selected_tracking_id is None:
            print(f"No trackable object found at click position ({frame_x}, {frame_y})")
    
    def tracker_changed(self, event=None):
        tracker = self.tracker_type.get()
        print(f"Tracker changed to: {tracker}")
        if tracker == "CSRT Tracking" and self.selected_tracking_id is not None:
            self.clear_tracking()
        elif tracker == "YOLO Tracking":
            self.csrt_tracker = None
            self.csrt_bbox = None
    ...
    def update_frame(self):
        ...
        try:
            ...
            elif current_time - self.last_frame_time > 2.0:
                self.display_placeholder()
                self.last_boxes = []
                self.last_tracking_info = None
                self.tracking_info_label.config(text="No tracking active")
